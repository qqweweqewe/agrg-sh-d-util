...
fn serial_exchange(bin_message: Vec<u8>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    // config or something??
...
pub fn serial_read(addr: Vec<u8>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
pub fn serial_write(addr: Vec<u8>, mut data: Vec<u8>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
pub fn bytes_to_datetime(raw: Vec<u8>) -> Result<String, Box<dyn std::error::Error>> {  
    Ok(format!("{:02X?}:{:02X?}:{:02X?} {:02X?}.{:02X?}.20{:02X?}", 
        raw[2], raw[1], raw[0], raw[4], raw[5], raw[6]))
}

pub fn datetime_to_bytes(datetime: String) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
pub fn get_datetime() -> Result<String, Box<dyn std::error::Error>> {
...
pub fn set_datetime(datetime: String) -> Result<Vec<u8>, Box<dyn std::error::Error>>{
